...
export default class PaymentUtil {
    ...
    static openPayment(appId, orderNo, amount) {
        var dateNow = PaymentUtil._datetostr(new Date(), "YYYYMMDDhhmmss", false);
        //amount = Number.toString(amount);
        var data = {
            ...
            JobCd:
                "CAPTURE" /*以下のいずれかを設定します。
                      ...
                      有効です。*/,
            ClientField1: appId.toString(),//申込ID
        };

        //ショップ情報チェック文字列
        var shopPassString = PaymentUtil._getShopPassString(data);
        data.ShopPassString = shopPassString;

        //新しいWindowを開き、そこにPOST
        PaymentUtil.postPaymentWindow(payUrl, data);
    }

    static postPaymentWindow(url, data) {
        ...
        // jquery("#temp_form").action = url;
        // jquery("#temp_form").method = "POST";
        // jquery("#temp_form").target = "new_window";
        jquery("#temp_form").attr("action", url);
        jquery("#temp_form").attr("method", "POST");
        jquery("#temp_form").attr("target", "_self");
    ...
    //ショップ情報チェック文字列
    static _getShopPassString(data) {
        var str = "";
    ...
    static _datetostr(date, format, is12hours) {
